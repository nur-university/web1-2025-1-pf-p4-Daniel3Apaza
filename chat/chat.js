function loadHeader() {
    return new Promise((resolve) => {
        // 1. Primero cargar todas las dependencias CSS con un ID único
        ...
        const fontsToLoad = [
            ...
            { 
                ...
                id: cssId // Usamos un ID único para forzar recarga
            }
        ];
        ...
        fontsToLoad.forEach(font => {
            // Eliminar estilos existentes con el mismo ID
            ...
            link.onerror = () => {
                ...
                // Si falla inicio.css, cargar estilos mínimos
                if (font.id === cssId) {
...
function loadHeaderHtml() {
    return fetch('/Inicio/Inicio.html?' + new Date().getTime()) // Evitar caché
        .then(res => {
        ...
        .then(html => {
            ...
            container.innerHTML = '';
            container.appendChild(header.cloneNode(true)); // Usar cloneNode para evitar problemas
            
            // Forzar repintado del header
            setTimeout(() => {
...
function loadHeaderScript() {
    return new Promise((resolve) => {
        ...
        headerJs.src = '/Inicio/Inicio.js?' + new Date().getTime(); // Evitar caché
        headerJs.onload = () => {
...
document.addEventListener('DOMContentLoaded', function () {
    loadHeader().then(() => {
        ...
        // Delegación global para que funcione siempre (independiente del ancho)
        ...
        container.addEventListener('click', function (e) {
            ...
            if (item && window.innerWidth <= 767) {
                console.log('Anuncio clickeado:', item); // ✅ DEBUG

                document.querySelector('.main_container').classList.add('show-chat');

                // Agregar botón "Volver" solo si no existe
        ...
        // Botón volver en la parte superior (si hace falta)
...
document.querySelectorAll('.imagen_precio').forEach(item => {
    item.style.cursor = 'pointer'; // opcional: mejora visual
